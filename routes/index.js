...
function bootstrap() {
    async.parallel({
        ...
        //  first filter
        let firstFilter = {}
        for(let coinName of Object.keys(reArrange)) {
            if(!reArrange[coinName]['buy'] || !reArrange[coinName]['sell']) continue
            if(!reArrange[coinName]['buy'].price || !reArrange[coinName]['sell'].price) continue
            if(reArrange[coinName]['buy'].price < reArrange[coinName]['sell'].price) continue
    
            firstFilter[coinName] = reArrange[coinName];
        }
